  ...
  class XGFile
    ...
    def parse_game_data
      ...
      until @decompressed_stream.eof?
        ...
        entry_type = record_data[8].unpack1("C") if record_data.size > 8
    ...
    def parse_header_match_record(data)
      # Simplified parsing - full implementation would parse all fields per XGFormat.txt
      {
        ...
        "MatchLength" => data[9 + 82, 4].unpack1("l<"),
        "RawData" => data[0, 100].unpack1("H*")  # First 100 bytes as hex for debugging
      }
    end
    ...
    def parse_header_game_record(data)
      {
        "EntryType" => 1,
        "Type" => "HeaderGame",
        "Score1" => data[9, 4].unpack1("l<"),
        "Score2" => data[13, 4].unpack1("l<"),
        "RawData" => data[0, 50].unpack1("H*")
      }
    end
